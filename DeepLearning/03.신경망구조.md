...
- SGD와 GD의 차이 
    ...
    <center>
    [출처](https://seamless.tistory.com/38) </center>
...
- SGD에 momentum 개념을 추가
    ...
    <p align = center><img src="https://latex.codecogs.com/svg.image?v_{t}&space;=&space;\gamma&space;v_{t-1}&space;-&space;\eta\frac{\partial}{\partial&space;W}&space;Loss(W)" title="v_{t} = \gamma v_{t-1} - \eta\frac{\partial}{\partial W} Loss(W)" /> </p>
    ...
    <p align=center><img src="https://latex.codecogs.com/svg.image?\gamma" title="\gamma" /> &nbsp; : momentum의 비중을 얼마나 줄 지 정해주는 hyper-parameter, 보통 0.9 사용</p>
...
- Momentum 방식은 현재 위치에서의 gradient와 mementum step을 독립적으로 계산
    - 멈춰야 할 시점에도 관성에 의해 훨씬 멀리 이동하여 최적의 parameter 찾기 힘든 경우 생김
- NAG 방식은 momentum step으로 이동한 후 gradeint를 구하여 이동

    <br>

    <p align=center><img src="https://latex.codecogs.com/svg.image?v_{t}&space;=&space;\gamma&space;v_{t-1}&plus;\eta&space;\frac{\partial}{\partial&space;W}Loss(W_{t}-\gamma&space;v_{t-1})" title="v_{t} = \gamma v_{t-1}+\eta \frac{\partial}{\partial W}Loss(W_{t}-\gamma v_{t-1})" /></p>
    <p align=center><img src="https://latex.codecogs.com/svg.image?W_{t&plus;1}=W_{t}&plus;v_{t}" title="W_{t+1}=W_{t}+v_{t}" /></p>

- 신경망에는 적합하지 않음
    - Gradient에 적용할 bias와 weight를 관성을 적용하여 계산하는 것이 역적파를 이용하여 계산을 더 효율적으로 사용하려는 것과 반대됨

- Bengio의 근사적 접근을 이용하면 편향과 가중치는 건들지 않기 때문에 신경망 학습 가능 
...
- learning rate decay를 신경쓰지 않아도 됨
    - 보통 0.01 정도로 설정한 후 그 이후로는 바꾸지 않음

    <br>

    <p align=center><img src="https://latex.codecogs.com/svg.image?G_{t}=G_{t-1}&plus;{\frac{\partial}{\partial&space;W}Loss(W)}\,&space;\odot&space;\,&space;{\frac{\partial}{\partial&space;W}Loss(W)}" title="G_{t}=G_{t-1}+{\frac{\partial}{\partial W}Loss(W)}\, \odot \, {\frac{\partial}{\partial W}Loss(W)}" /></p>

    <center><img src="https://latex.codecogs.com/svg.image?G_{t}" title="G_{t}" /> &nbsp; : 업데이트 할 파라미터 개수의 차원벡터, 이동한거리를 나타내는 척도로 사용</center>
    <center>t까지 각변수가 이동한 Gradient의 sum of squares 값 저장</center>
...
- G값은 제곱한 값이기 때문에 학습이 계속되면 learning rate가 매우 작아져 값이 update 되지 않는 경우가 생김

<br>

### RMSProp
- Adagrad의 단점을 보완하기 위해 제안된 방법 ㅎ
- Adagrad의 식에서 <img src="https://latex.codecogs.com/svg.image?G_{t}" title="G_{t}" /> 부분을 합이 아니라 지수 평균으로 대체한 것이 핵심 
    ...
    <center><img src="https://latex.codecogs.com/svg.image?\gamma" title="\gamma" /> &nbsp;: 지수 이동 평균에서의 forgetting factor, decaying factor</center>
    <center>hyper-parameter의 값으로 보통 0.9 ~ 0.999의 값 가짐 </center>
...
- RMSProp과 Momentum 방식을 융합한 방법
    - Momentum과 유사하게 지금까지 계산한 기울기의 지수 평균 저장
    - RMSProp과 유사하기 기울기의 제곱값의 지수 평균 저장 

    <br>

    <p align=center><img src="https://latex.codecogs.com/svg.image?m_{t}=\beta_{1}m_{t-1}\,&space;&plus;&space;\,&space;(1-\beta_{1})\frac{\partial}{\partial&space;W}Loss(W_{t})" title="m_{t}=\beta_{1}m_{t-1}\, + \, (1-\beta_{1})\frac{\partial}{\partial W}Loss(W_{t})" /></p>

    <p align=center><img src="https://latex.codecogs.com/svg.image?v_{t}=\beta_{2}v_{t-1}\,&space;&plus;&space;\,&space;(1-\beta_{2}){\frac{\partial}{\partial&space;W}Loss(W_{t})}\odot&space;{\frac{\partial}{\partial&space;W}Loss(W_{t})}" title="v_{t}=\beta_{2}v_{t-1}\, + \, (1-\beta_{2}){\frac{\partial}{\partial W}Loss(W_{t})}\odot {\frac{\partial}{\partial W}Loss(W_{t})}" /></p>
    <center> **여기서도 element wise 연산인지 확인해볼것**<center>

    <img src="https://latex.codecogs.com/svg.image?W_{t&plus;1}=W_{t}-m_{t}\frac{\eta}{\sqrt{v_{t}&plus;\epsilon}}" title="W_{t+1}=W_{t}-m_{t}\frac{\eta}{\sqrt{v_{t}+\epsilon}}" />

    <br>

- <img src="https://latex.codecogs.com/svg.image?m" title="m" /> 과 &nbsp;<img src="https://latex.codecogs.com/svg.image?v" title="v" /> 처음에 0으로 초기화 
- 0에 가깝게 bias 되어 있을 것이라고 판단하여 먼저 이를 unbiased 하게 만들어주는 과정 필요
    - 아래 식을 통하여 unbiased된 expectation 얻음 
