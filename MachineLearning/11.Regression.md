...
<p align=center><img src="https://latex.codecogs.com/svg.image?\hat{y_i}&space;=&space;w_1&space;x_{i1}&space;&plus;&space;w_2&space;x_{i2}...&space;&plus;&space;w_{p}&space;x_{ip}&space;&plus;&space;b" title="\hat{y_i} = w_1 x_{i1} + w_2 x_{i2}... + w_{p} x_{ip} + b" /></p>


>- <img src="https://latex.codecogs.com/svg.image?\hat{y_i}" title="\hat{y_i}" /> &nbsp; : &nbsp; 예측값
>- <img src="https://latex.codecogs.com/svg.image?x" title="x" /> &nbsp; : &nbsp; 특성(feature-컬럼)
>- <img src="https://latex.codecogs.com/svg.image?w" title="w" /> &nbsp; : &nbsp;  가중치(weight), 회귀계수(regression coefficient), 특성이 &nbsp; <img src="https://latex.codecogs.com/svg.image?\hat{y_i}" title="\hat{y_i}"> 에 얼마나 영향을 주는지 정도
>- <img src="https://latex.codecogs.com/svg.image?b" title="b" /> &nbsp; : &nbsp; 절편
>- <img src="https://latex.codecogs.com/svg.image?p" title="p" />  &nbsp; : &nbsp; p 번째 특성(feature)/p번째 가중치
>- <img src="https://latex.codecogs.com/svg.image?i" title="i" /> &nbsp; : &nbsp; i번째 관측치(sample)
...
## Ridge Regression
- 손실함수(loss function)에 규제항으로 <img src="https://latex.codecogs.com/svg.image?\alpha&space;\sum_{i=1}^{n}{w_{i}^{2}}" height=30 title="\alpha \sum_{i=1}^{n}{w_{i}^{2}}" /> &nbsp; (L2 Norm)을 더해줌
- <img src="https://latex.codecogs.com/svg.image?\alpha" title="\alpha" />는 하이퍼파라미터로 모델을 얼마나 많이 규제할지 조절
    - <img src="https://latex.codecogs.com/svg.image?\alpha&space;=&space;0" title="\alpha = 0" />  에 가까울수록 규제가 약해짐 
        - 0일 경우 선형 회귀동일
    - <img src="https://latex.codecogs.com/svg.image?\alpha" title="\alpha" /> 가 커질 수록 모든 가중치가 작아져 입력데이터의 Feature들 중 중요하지 않은 Feature의 output에 대한 영향력이 작아짐
...
- MES에 규제항을 더해줌
    - 규제항 
        - 오차를 키워줌
        - <img src="https://latex.codecogs.com/svg.image?w" title="w" />의 영향력을 작게 만들어 feature의 영향력을 낮춤
    - 오차는 <img src="https://latex.codecogs.com/svg.image?w" title="w" />를 잘못찾아서 발생
    - <img src="https://latex.codecogs.com/svg.image?\alpha" title="\alpha" />  는 우리가  control 할 수 있는 hyper-parameter
    - 규제를 통해 feature를 selection하는 것이 목적! -> 오차를 크게 만들어줌 -> 그러기 위해서 w를 작게 만들어줌


<p align=center><img src="https://latex.codecogs.com/svg.image?\text{Loss&space;Function}(w)&space;=&space;\text{MSE}(w)&space;&plus;&space;\alpha&space;\cfrac{1}{2}\sum_{i=1}^{n}{w_{i}^{2}}" title="\text{Loss Function}(w) = \text{MSE}(w) + \alpha \cfrac{1}{2}\sum_{i=1}^{n}{w_{i}^{2}}" /></p>

- 0에 가깝게는 되는데 절대 0이 되지 않음
- 보통 Rasson 보다는 Ridge를 많이 사용
...
- 손실함수에 규제항으로 <img src="https://latex.codecogs.com/svg.image?\alpha&space;\sum_{i=1}^{n}{\left|&space;w_i&space;\right|}"  height=30 title="\alpha \sum_{i=1}^{n}{\left| w_i \right|}" /> (L1 Norm)더함
- Lasso 회귀의 상대적으로 덜 중요한 특성의 가중치를 0으로 만들어 자동으로 Feature Selection 만들어줌

<p align=center><img src="https://latex.codecogs.com/svg.image?\text{Loss&space;Function}(w)&space;=&space;\text{MSE}(w)&space;&plus;&space;\alpha&space;\sum_{i=1}^{n}{\left|&space;w_i&space;\right|}" title="\text{Loss Function}(w) = \text{MSE}(w) + \alpha \sum_{i=1}^{n}{\left| w_i \right|}" /></p>
...
- 혼합뷰율 <img src="https://latex.codecogs.com/svg.image?r" title="r" /> 을 사용해 혼합정도를 조절
- <img src="https://latex.codecogs.com/svg.image?r=0" title="r=0" /> 이면 릿지와 같고 <img src="https://latex.codecogs.com/svg.image?r=1" title="r=1" /> 이면 라쏘와 같음

<p align=center><img src="https://latex.codecogs.com/svg.image?\text{Loss&space;Function}(w)&space;=&space;\text{MSE}(w)&space;&plus;&space;r\alpha&space;\sum_{i=1}^{n}{\left|&space;w_i&space;\right|}&space;&plus;&space;\cfrac{1-r}{2}\alpha\sum_{i=1}^{n}{w_{i}^{2}}" title="\text{Loss Function}(w) = \text{MSE}(w) + r\alpha \sum_{i=1}^{n}{\left| w_i \right|} + \cfrac{1-r}{2}\alpha\sum_{i=1}^{n}{w_{i}^{2}}" /></p>
...
<p align=center><img src="https://latex.codecogs.com/svg.image?\hat{p}&space;=&space;\sigma&space;\left(&space;\mathbf{W}^{T}&space;\cdot&space;\mathbf{x}&space;\right)\\" title="\hat{p} = \sigma \left( \mathbf{W}^{T} \cdot \mathbf{x} \right)\\" /></p>



>- <img src="https://latex.codecogs.com/svg.image?\sigma()\" title="\sigma()\" />  &nbsp; : &nbsp; logistic함수
>- <img src="https://latex.codecogs.com/svg.image?\mathbf{W}\" title="\mathbf{W}\" /> &nbsp; : &nbsp;가중치
>- <img src="https://latex.codecogs.com/svg.image?\mathbf{x}" title="\mathbf{x}" /> &nbsp; : &nbsp; 입력 특성
...
<p align=center><img src="https://latex.codecogs.com/svg.image?\sigma(x)&space;=&space;\frac{1}{1&space;&plus;&space;\mathbf{e}^{-x}}" title="\sigma(x) = \frac{1}{1 + \mathbf{e}^{-x}}" /></p>
...
<p align=center><img src="https://latex.codecogs.com/svg.image?\hat{y}&space;=&space;\begin{cases}&space;0\quad\hat{p}<0.5\\1\quad\hat{p}\geqq0.5&space;\end{cases}" title="\hat{y} = \begin{cases} 0\quad\hat{p}<0.5\\1\quad\hat{p}\geqq0.5 \end{cases}" /></p>
...
<p align=center><img src="https://latex.codecogs.com/svg.image?L(\mathbf{W})&space;=&space;-&space;\frac{1}{m}&space;\sum_{i=1}^{m}{\left[&space;y_{i}&space;\log{\left(&space;\hat{p}_i&space;\right)}&space;&plus;&space;\left(&space;1&space;-&space;y_i&space;\right)&space;\log{\left(&space;1&space;-&space;\hat{p}_i&space;\right)}&space;\right]}\\" title="L(\mathbf{W}) = - \frac{1}{m} \sum_{i=1}^{m}{\left[ y_{i} \log{\left( \hat{p}_i \right)} + \left( 1 - y_i \right) \log{\left( 1 - \hat{p}_i \right)} \right]}\\" /></p>

>- <img src="https://latex.codecogs.com/svg.image?y" title="y" />  &nbsp; : &nbsp; 실제값
>- <img src="https://latex.codecogs.com/svg.image?\hat{p}\" title="\hat{p}\" /> &nbsp; : &nbsp; 예측확률

<br>

- 손실이 커져야하기 때문에 -를 곱해줌
- y(실제값) 이 1인 경우
    - <img src="https://latex.codecogs.com/svg.image?y_{i}\log{\left(\hat{p}_i\right)}" title="y_{i}\log{\left(\hat{p}_i\right)}" /> &nbsp; 이 손실을 계산
    - 맞았다면 $\hat{p}$ 이 0.5 이상
- y가 0인 경우 
    - <img src="https://latex.codecogs.com/svg.image?\left(&space;1&space;-&space;y_i&space;\right)&space;\log{\left(&space;1&space;-&space;\hat{p}_i&space;\right)}" title="\left( 1 - y_i \right) \log{\left( 1 - \hat{p}_i \right)}" /> &nbsp;이 손실을 계산
- <img src="https://latex.codecogs.com/svg.image?\hat{p}\" title="\hat{p}\" />(예측확률)이 클수록 반환값은 작아지고 작을 수록 값이 커짐
...
- 로그 손실을 <img src="https://latex.codecogs.com/svg.image?\mathbf{W}" title="\mathbf{W}" />로 미분
    - 아래 도함수로 기울기를 구해 기울기가 0이 될 때 까지 W(가중치)들을 update

<br>

<p align=center><img src="https://latex.codecogs.com/svg.image?\frac{\partial}{\partial&space;w_j}&space;=&space;\frac{1}{m}&space;\sum_{i=1}^{m}{\left(&space;\sigma&space;\left(&space;\mathbf{W}^{T}&space;\cdot&space;\mathbf{x}_i&space;\right)&space;-&space;\mathbf{y}_i&space;\right)}&space;x_{ij}" title="\frac{\partial}{\partial w_j} = \frac{1}{m} \sum_{i=1}^{m}{\left( \sigma \left( \mathbf{W}^{T} \cdot \mathbf{x}_i \right) - \mathbf{y}_i \right)} x_{ij}" /></p>
